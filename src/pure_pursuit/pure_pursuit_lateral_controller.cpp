...
}; //管理参数调试。速度、曲率、横向误差对前瞻距离的贡献
}  // namespace

namespace pure_pursuit
{
PurePursuitLateralController::PurePursuitLateralController(rclcpp::Node & node)//重载函数
: clock_(node.get_clock()), //ros2时钟初始化。
  ...
  tf_listener_(tf_buffer_)
{
  ...
  param_.max_steering_angle = vehicle_info.max_steer_angle_rad;//获取车辆参数信息
...
double PurePursuitLateralController::calcLookaheadDistance(//计算前瞻距离
  const double lateral_error, const double curvature, const double velocity, const double min_ld,
  const bool is_control_cmd)
{
  const double vel_ld = abs(param_.ld_velocity_ratio * velocity);//速度对前瞻距离的影响
  const double curvature_ld = -abs(param_.ld_curvature_ratio * curvature);//曲率对前瞻距离的影响
  ...
  if (abs(lateral_error) >= param_.long_ld_lateral_error_threshold) {
    ...
    lateral_error_ld = abs(param_.ld_lateral_error_ratio * lateral_error);//横向误差对前瞻距离的影响（仅大于阈值时使用）
  }
...
TrajectoryPoint PurePursuitLateralController::calcNextPose(//预测下一个位姿
...
{
  // Calculate current curvature（曲率计筭）
...
void PurePursuitLateralController::averageFilterTrajectory(//平滑轨迹
